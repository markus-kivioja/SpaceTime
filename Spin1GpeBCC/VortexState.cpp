...
bool VortexState::load(const std::string &path)
{
	...
	fs.read((char*)&m_c0, sizeof(ddouble));  // effective interaction strength
	fs.read((char*)&m_c1, sizeof(ddouble));  // effective interaction strength
...
bool VortexState::save(const std::string &path) const
{
	...
	fs.write((char*)&m_c0, sizeof(ddouble));  // effective interaction strength
	fs.write((char*)&m_c1, sizeof(ddouble));  // effective interaction strength
...
void VortexState::iterateSolution(ddouble (*potential)(const ddouble, const ddouble), const uint maxIters, const ddouble epsilon)
{
	uint i, j, k, ii;
	const ddouble PIx2drdz = PIx2 * m_rstep * m_zstep;
	const ddouble kappaSqr = 0.5 * ddouble(m_kappa * m_kappa);
	...
	for (k = 0; k < maxIters; k++)
	{
		...
		{
			const ddouble r = m_rplug + i * m_rstep;
			const ddouble lapl = kappaSqr / (r * r) + 2.0 * (laplr + laplz);
			const ddouble gradr = 0.25 / (r * m_rstep);
			ddouble dv = PIx2drdz * r;
			for (j = 0; j < m_zsize; j++, ii++)
			{
				const ddouble pot = potential(r, j * m_zstep) + (m_c0 + m_c1) * m_f[ii] * m_f[ii];
		...
		if (k % 1000 == 0)
		{
			std::cout << "iter: " << k << ", diff: " << diff << ", prob: " << prob << std::endl;
		}
		if (diff < epsilon)
		{
			break;
...
ddouble VortexState::getFunction(const ddouble r, const ddouble z) const
{
	...
	if (rr < 0.0) // radius is smaller than m_rplug
	{
		return 0.0;
	}
	const uint ir = uint(rr);
	if (ir >= m_rsize)  // radius is out of boundaries
	{
		return 0.0;
	}
	...
	if (iz >= m_zsize) // z is out of boundaries
	{
		return 0.0;
	}
...
ddouble VortexState::integrateZ(const ddouble vol) const
{
	if(vol <= 0.0) return 0.0;
	uint i, j;
	ddouble sum = 0.0;
	const ddouble PIx4drdz = 4.0 * PI * m_rstep * m_zstep;
	for(j=0; j<m_zsize; j++)
	{
		const ddouble prev = sum;
		for(i=1; i<m_rsize; i++)
		{
			const uint ii = i * m_zsize + j;
			const ddouble r = m_rplug + i * m_rstep;
			const ddouble f = 0.5 * (m_f[ii] + (j + 1 < m_zsize ? m_f[ii+1] : 0.0));
			sum += PIx4drdz * r * f * f;
		}
		if(sum > vol) return (j + (vol - prev) / (sum - prev)) * m_zstep;
	}
	return m_zsize * m_zstep;
}
