...
__global__ void itp_psi(PitchedPtr HPsiPtr, PitchedPtr nextStep, PitchedPtr prevStep, PitchedPtr qs, const int2* __restrict__ d1Ptr, const myFloat* __restrict__ hodges, MagFields Bs, const uint3 dimensions, const myFloat block_scale, const myFloat3 p0, const myFloat c0, const myFloat c2, myFloat dt)
{
...
#pragma unroll
	...
	const myFloat totalPot = trap(globalPos) + c0 * normSq;
...
#pragma unroll
	...
	const myFloat2 magXY = SQRT_2 * (conj(prev.s1) * prev.s0 + conj(prev.s0) * prev.s_1);
	myFloat3 B = c2 * myFloat3{ magXY.x, magXY.y, normSq_s1 - normSq_s_1 };
	B += magneticField(globalPos, Bs.Bq, Bs.Bb);
	...
	nextPsi->values[dualNodeId].s_1 += 2.0 * dt * myFloat2{ H.s_1.y, -H.s_1.x };
};
...
__global__ void maxHamilton(myFloat* maxHamlPtr, PitchedPtr prevStep, MagFields Bs, uint3 dimensions, myFloat block_scale, myFloat3 p0, myFloat c0, myFloat c2)
{
	const size_t xid = blockIdx.x * blockDim.x + threadIdx.x;
	const size_t yid = blockIdx.y * blockDim.y + threadIdx.y;
	const size_t zid = blockIdx.z * blockDim.z + threadIdx.z;
...
__global__ void innerProduct(myFloat2* result, PitchedPtr pLeft, PitchedPtr pRight, uint3 dimensions)
{
	size_t xid = blockIdx.x * blockDim.x + threadIdx.x;
	size_t yid = blockIdx.y * blockDim.y + threadIdx.y;
	size_t zid = blockIdx.z * blockDim.z + threadIdx.z;
...
__global__ void densDiff(myFloat* result, PitchedPtr pLeft, PitchedPtr pRight, uint3 dimensions)
{
	...
	myFloat leftSqr = (conj(left.s1) * left.s1).x + (conj(left.s0) * left.s0).x + (conj(left.s_1) * left.s_1).x;
	myFloat rightSqr = (conj(right.s1) * right.s1).x + (conj(right.s0) * right.s0).x + (conj(right.s_1) * right.s_1).x;

	myFloat diff = leftSqr - rightSqr;
...
__global__ void com(myFloat3* com, PitchedPtr prevStep, uint3 dimensions, const myFloat block_scale, const myFloat3 p0)
{
	...
	myFloat dens = (psi.s1 * conj(psi.s1)).x + (psi.s0 * conj(psi.s0)).x + (psi.s_1 * conj(psi.s_1)).x;
	const myFloat3 localPos = d_localPos[dualNodeId];
	const myFloat3 globalPos = { p0.x + block_scale * (dataXid * BLOCK_WIDTH_X + localPos.x),
		p0.y + block_scale * (yid * BLOCK_WIDTH_Y + localPos.y),
		p0.z + block_scale * (zid * BLOCK_WIDTH_Z + localPos.z) };
